...
// 프로젝트 ID → 이미지 파일명 매핑 (resource/ 폴더의 한글 파일명 기준)
const PROJECT_IMAGES: Record<string, string[]> = {
  'ai-mata': ['초등마타.png', '중고등마타.png', 'AI마타_선생님.png', 'AI마타_학생.png'],
  paddly: ['패들리.png'],
  'cool-hanja': ['쿨한자.png'],
};

// public/images/ 경로로 변환 (한글 파일명 URL 인코딩)
...
```
PROJECT_IMAGES[c.id].length === 0 → CardImagePlaceholder (240px 그라디언트 + 프로젝트명)
PROJECT_IMAGES[c.id].length > 0   → ProjectImageSlider 사용
...
**PROJECT_IMAGES 맵** — card-slider.tsx 상단에 하드코딩:
```typescript
const PROJECT_IMAGES: Record<string, string[]> = {
  'project-id': ['파일명.png'],  // images 없으면 []
};
const getImgSrc = (filename: string) => `/images/${encodeURIComponent(filename)}`;
```
